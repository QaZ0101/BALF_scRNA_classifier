...
rm(list=ls())
data<-read.csv(file = "mild_severe.csv")
data<-as.matrix(data)
...
data<-t(data)
dim(data)
true_lables<-read.csv(file="mild_severe_label.csv")
...
write.table(Ordered_diff_test_res_Nor, file = "mild_vs_severe.txt", sep = '\t', row.names = TRUE, col.names = TRUE)
